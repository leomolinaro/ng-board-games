...
export class BaronyRemoteService {
  ...
  private players(gameId: string) {
    return this.cloud.collection<BaronyPlayerDoc>(`barony-games/${gameId}/players`);
  }
  getPlayers(gameId: string, queryFn?: BgCloudCollectionQuery<BaronyPlayerDoc> | undefined) {
    return this.cloud.getAll(this.players(gameId), queryFn);
  }
  selectPlayers$(gameId: string, queryFn?: BgCloudCollectionQuery<BaronyPlayerDoc> | undefined) {
  ...
  insertPlayer$(player: BaronyPlayerDoc, gameId: string): Observable<BaronyPlayerDoc> {
    return this.cloud.set$(player.id, player, this.players(gameId));
  }
  updatePlayer$(patch: Partial<BaronyPlayerDoc>, playerId: string, gameId: string) {
  ...
  private stories(gameId: string) {
    return this.cloud.collection<BaronyStoryDoc>(`barony-games/${gameId}/stories`);
  }
  getStories(gameId: string, queryFn?: BgCloudCollectionQuery<BaronyStoryDoc> | undefined) {
  ...
  selectStories$(gameId: string, queryFn?: BgCloudCollectionQuery<BaronyStoryDoc> | undefined) {
