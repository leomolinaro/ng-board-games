...
export function getVillageDestroyedPlayer(
  ...
  const villagePawn = land.pawns.find(
    (p) => p.type === 'village' && p.color !== player.color
  ) as BaronyPawn;
  return game
    .getPlayers()
    .find((p) => p.color === villagePawn.color) as BaronyPlayer;
...
export function isRecruitmentValid(
  ...
  const hasValidLandTiles = game
    .getLands()
    .some((lt) =>
      lt.pawns.some((p) => p.color === player.color && p.type === 'city')
    );
...
function isValidLandForNewCity(
  ...
  const player = game.getPlayer(playerId);
  return (
    land.pawns.some((p) => p.type === 'village' && p.color === player.color) &&
    land.type !== 'forest' &&
