...
export function getValidLandsForRecruitment (playerId: BaronyColor, game: BaronyGameStore): BaronyLand[] {
  ...
  const validLandTiles = game.getLands ()
...
export function getValidSourceLandsForFirstMovement (player: BaronyColor, game: BaronyGameStore): BaronyLand[] {
  return game.getLands ()
    .filter ((land) => isValidFirstMovementSource (land, player, game));
}

export function getValidSourceLandsForSecondMovement (player: BaronyColor, firstMovement: BaronyMovement, game: BaronyGameStore): BaronyLand[] {
  return game.getLands ()
    .filter ((land) => isValidSecondMovementSource (land, player, firstMovement, game));
}

export function getValidTargetLandsForMovement (movementSource: BaronyLandCoordinates, player: BaronyColor, game: BaronyGameStore): BaronyLand[] {
  return getNearbyLands (movementSource, game)
    .filter ((lt) => isValidMovementTarget (lt, player, game));
}
...
export function getValidBuildingsForConstruction (playerId: BaronyColor, game: BaronyGameStore): ("stronghold" | "village")[] {
  const player = game.getPlayer (playerId);
...
export function getMaxKnightForRecruitment (land: BaronyLandCoordinates, playerId: BaronyColor, game: BaronyGameStore): number {
  const player = game.getPlayer (playerId);
...
export function getVillageDestroyedPlayer (landCoordinates: BaronyLandCoordinates, playerId: BaronyColor, game: BaronyGameStore): BaronyPlayer {
...
export function isSecondMovementValid (player: BaronyColor, firstMovement: BaronyMovement, game: BaronyGameStore): boolean {
  return game
    .getLands ()
    .some ((land) =>
...
function isValidSecondMovementSource (land: BaronyLand, player: BaronyColor, firstMovement: BaronyMovement, game: BaronyGameStore) {
  return (
    ...
    getNearbyLands (land.coordinates, game).some ((nlt) =>
      isValidMovementTarget (nlt, player, game)
    )
  );
}
...
function isValidMovementTarget (land: BaronyLand, playerId: BaronyColor, game: BaronyGameStore) {
  ...
  return (
    ...
    !(
      land.type === "mountain" &&
      hasOneOrMoreOpponentKnight (land.coordinates, player.id, game)
    )
  );
}
...
function isValidLandForExpedition (landCoordinates: BaronyLandCoordinates, playerId: BaronyColor, game: BaronyGameStore) {
  const land = game.getLand (landCoordinates);
  return (
    land.type !== "lake" &&
    !land.pawns.length &&
...
export function isConflict (landCoordinates: BaronyLandCoordinates, playerId: BaronyColor, game: BaronyGameStore): boolean {
  const land = game.getLand (landCoordinates);
  const player = game.getPlayer (playerId);
  return (
    land.pawns.some ((p) => p.color !== player.id) &&
    land.pawns.some ((p) => p.color === player.id)
  );
}

export function isVillageBeingDestroyed (landCoordinates: BaronyLandCoordinates, playerId: BaronyColor, game: BaronyGameStore): boolean {
  const land = game.getLand (landCoordinates);
  const player = game.getPlayer (playerId);
  return land.pawns.some (
    (p) => p.type === "village" && p.color !== player.id
  );
}
