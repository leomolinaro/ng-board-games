...
export function getValidTargetLandsForMovement(
  ...
  return getNearbyLands(movementSource, game).filter((lt) =>
    isValidMovementTarget(lt, player, game),
  );
}

export function getValidLandsForConstruction(
  player: BaronyColor,
  game: BaronyGameStore,
): BaronyLand[] {
  return game
...
function getPlayerVictoryPoints(player: BaronyPlayer): number {
...
export function isRecruitmentValid(
  playerId: BaronyColor,
  game: BaronyGameStore,
): boolean {
  const player = game.getPlayer(playerId);
...
function isValidFirstMovementSource(
  land: BaronyLand,
  player: BaronyColor,
  game: BaronyGameStore,
): boolean {
...
function isValidMovementTarget(
  land: BaronyLand,
  playerId: BaronyColor,
  game: BaronyGameStore,
): boolean {
...
export function isConstructionValid(
  player: BaronyColor,
  game: BaronyGameStore,
): boolean {
  return game
...
export function isNewCityValid(
  player: BaronyColor,
  game: BaronyGameStore,
): boolean {
  return (
...
export function isExpeditionValid(
  player: BaronyColor,
  game: BaronyGameStore,
): boolean {
  return (
...
export function isNobleTitleValid(
  playerId: BaronyColor,
  game: BaronyGameStore,
): boolean {
...
function getPlayerResourcePoints(
  playerId: BaronyColor,
  game: BaronyGameStore,
): number {
  const player = game.getPlayer(playerId);
...
export function isPlayerWinning(
  playerId: BaronyColor,
  game: BaronyGameStore,
): boolean {
  const player = game.getPlayer(playerId);
...
export function getResourcePoints(resource: BaronyResourceType): number {
  switch (resource) {
    case 'fields':
      return 5;
    case 'plain':
...
export function getResourceVicotryPoints(resource: BaronyResourceType): number {
  switch (resource) {
    case 'fields':
      return 3;
    case 'plain':
