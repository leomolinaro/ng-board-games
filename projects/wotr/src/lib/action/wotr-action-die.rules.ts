...
export class WotrActionDieRules {
  ...
  private FREE_PEOPLES_ACTION_DICE: WotrFreePeopleActionDie[] = [
    "character", "muster", "event", "muster-army", "will-of-the-west"
  ];

  private SHADOW_ACTION_DICE: WotrShadowActionDie[] = [
    "character", "army", "event", "muster", "muster-army", "eye"
  ];
  ...
  rollableActionDice (frontId: WotrFrontId): number {
    switch (frontId) {
      case "free-peoples": {
        let nDice = 4;
        if (this.charaters.isInPlay ("aragorn")) { nDice += 1; }
        if (this.charaters.isInPlay ("gandalf-the-white")) { nDice += 1; }
        return nDice;
      }
      case "shadow": {
        let nDice = 7;
