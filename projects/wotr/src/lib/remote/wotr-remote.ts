...
import type { BgCloudCollection, BgCloudCollectionQuery } from '@leobg/commons';
...
export class WotrRemoteService {
  ...
  private games(): BgCloudCollection<WotrGameDoc> {
    return this.cloud.collection<WotrGameDoc>('wotr-games');
  }
  async getGame(gameId: string): Promise<WotrGameDoc | undefined> {
    return this.cloud.get(gameId, this.games());
  }
  selectGames$(
    queryFn?: BgCloudCollectionQuery<WotrGameDoc>,
  ): Observable<WotrGameDoc[]> {
    return this.cloud.selectAll$(this.games(), queryFn);
  }
  insertGame(game: WotrGameDoc): Promise<WotrGameDoc> {
    return this.cloud.set(game.id, game, this.games());
  }
  updateGame(patch: Partial<WotrGameDoc>, gameId: string): Promise<void> {
  ...
  private players(gameId: string): BgCloudCollection<WotrPlayerDoc> {
  ...
  selectPlayer$(
    playerId: string,
    gameId: string,
  ): Observable<WotrPlayerDoc | undefined> {
    return this.cloud.select$(playerId, this.players(gameId));
  }
  insertPlayer(player: WotrPlayerDoc, gameId: string): Promise<WotrPlayerDoc> {
  ...
  private stories(gameId: string): BgCloudCollection<WotrStoryDoc> {
