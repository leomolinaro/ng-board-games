    ...
    switch (l.type) {
      ...
      case "action": {
        ...
        if ("character" in l.story && l.story.character) {
          parsed.push(this.string(", using "), this.character(l.story.character), this.string("'s ability"));
        }
        if ("die" in l.story) {
      ...
      case "story": {
        switch (l.story.type) {
          ...
          case "reaction-card-skip":
            return [this.player(l.front), this.string(" skip "), this.card(cardToLabel(l.story.card))];
          case "reaction-character-skip":
            return [
              this.player(l.front),
              this.string(" skip character reaction") /* , this.card (combatCardToLabel (l.story.character)) */
            ];
          case "reaction-combat-card-skip":
            return [this.player(l.front), this.string(" skip "), this.card(combatCardToLabel(l.story.card))];
          default:
