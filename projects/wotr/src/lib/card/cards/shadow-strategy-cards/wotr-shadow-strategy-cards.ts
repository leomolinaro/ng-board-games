  ...
  createCard(cardId: WotrShadowStrategyCardId): WotrEventCard {
    switch (cardId) {
        ...
        return {
          ...
          effect: async (params) => {
            ...
            while (regions.length > 0) {
              ...
              const rollStory = await this.shadow.rollCombatDice(
                Math.min(nElvenArmyUnits, 5),
              );
              if (!('actions' in rollStory))
                throw new Error('Unexpected story');
              ...
              regions = immutableUtil.listRemoveFirst(
        ...
        return {
          canBePlayed: () =>
            ...
            this.q
              ...
              .some(
                (region) =>
                  region.isUnderSiege('free-peoples') &&
                  region.hasArmyUnitsOfNation('isengard'),
              ),
          play: async (ui) => {
      ...
      case 'sstr03':
        return {
          canBePlayed: () =>
            this.q.region('minas-tirith').isUnderSiege('free-peoples'),
          play: async (ui) => {
            ...
            if (this.unitUtils.hasLeaders(fpArmy)) {
              ...
              if (units.leaders?.length) {
                actions.push(
                  eliminateLeader('minas-tirith', units.leaders[0].nation),
                );
              }
            }
            return actions;
      ...
      case 'sstr06':
        return {
          canBePlayed: () =>
            this.q.regions().some((region) => this.isStormcrowRegion(region)),
          play: async (ui) => {
        ...
        return {
          play: async (ui) => {
            ...
            while (shouldContinueMoving) {
              ...
              const movActions = await ui.unitUi.moveThisArmyTo(
                units,
                'shadow',
                toRegionId,
              );
      ...
      case 'sstr11':
        return {
          ...
          play: async (ui) => {
            const dunlandRegions: WotrRegionId[] = [
              'north-dunland',
              'south-dunland',
            ];
            const regions = this.q
      ...
      case 'sstr12':
        return {
          ...
          play: async (ui) => {
            ...
            actions.push(
              moveCharacters(witchKingRegion.id, 'angmar', 'the-witch-king'),
            );
      ...
      case 'sstr14':
        return {
          ...
          play: async (ui) => {
            ...
            const unit = await ui.askReinforcementUnit(
              'Select a Sauron unit to recruit',
              {
                canPass: false,
                frontId: 'shadow',
                units: [
      ...
      case 'sstr15':
        return {
          ...
          play: async (ui) => {
            const actions: WotrAction[] = [];
            for (let i = 0; i < 2; i++) {
              if (!this.q.nation('sauron').nEliteReinforcements())
                return actions;
              const regions = this.q
                .regions()
        ...
        return {
          ...
          play: async (ui) => {
            ...
            if (nElites === 1 && nRegulars <= 5) {
              const eliteRecruitment = await ui.unitUi.recruitEliteByCard(
                'orthanc',
                'isengard',
              );
              if (eliteRecruitment) {
      ...
      case 'sstr18':
        return {
          ...
          play: async (ui) =>
            ui.unitUi.recruitUnitsInSameRegionByCard(
              'minas-morgul',
              'sauron',
              5,
              0,
