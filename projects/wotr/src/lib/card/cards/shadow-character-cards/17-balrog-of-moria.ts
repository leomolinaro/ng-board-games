...
const balrogOfMoria01: WotrScenario = {
  ...
  name: 'Balrog of Moria',
  description:
    'When The Fellowship is declared through Moria and the Balrog card is in play',
  loadDefinition: () => ({
    setup: (setupBuilder) =>
      setupBuilder.shuffledDecks().fellowshipProgress(4).huntPool('1').build(),
...
const balrogOfMoria02: WotrScenario = {
  ...
  name: 'Balrog of Moria',
  description:
    'When the Balrog card is in play and the Shadow is battling near Moria',
  loadDefinition: () => ({
    setup: (setupBuilder) =>
      setupBuilder
        .shuffledDecks()
