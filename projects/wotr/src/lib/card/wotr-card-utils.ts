...
interface ExpansionCards {
  ...
  fpstrReplacements: Partial<
    Record<WotrFreePeoplesStrategyCardId, WotrFreePeoplesStrategyCardId>
  >;
  fpstrAdditions: WotrFreePeoplesStrategyCardId[];
  schaReplacements: Partial<
    Record<WotrShadowCharacterCardId, WotrShadowCharacterCardId>
  >;
  schaAdditions: WotrShadowCharacterCardId[];
  sstrReplacements: Partial<
    Record<WotrShadowStrategyCardId, WotrShadowStrategyCardId>
  >;
...
export class WotrCardUtils {
  ...
  private getGameExpansionCards(options: WotrGameOptions): ExpansionCards {
    ...
    if (expansions.includes('kome'))
      expansionCards.push(this.komeExpansionCards);
    ...
    for (const expansionCard of expansionCards) {
      Object.assign(
        gameExpansionCards.fpchaReplacements,
        expansionCard.fpchaReplacements,
      );
      gameExpansionCards.fpchaAdditions.push(...expansionCard.fpchaAdditions);
      Object.assign(
        gameExpansionCards.fpstrReplacements,
        expansionCard.fpstrReplacements,
      );
      gameExpansionCards.fpstrAdditions.push(...expansionCard.fpstrAdditions);
      Object.assign(
        gameExpansionCards.schaReplacements,
        expansionCard.schaReplacements,
      );
      gameExpansionCards.schaAdditions.push(...expansionCard.schaAdditions);
      Object.assign(
        gameExpansionCards.sstrReplacements,
        expansionCard.sstrReplacements,
      );
  ...
  getCardDecks(options: WotrGameOptions): WotrCardDecks {
    ...
    for (const addition of expansionCards.fpchaAdditions)
      fpchaCardIds.push(addition);
    for (const addition of expansionCards.fpstrAdditions)
      fpstrCardIds.push(addition);
    for (const addition of expansionCards.schaAdditions)
      schaCardIds.push(addition);
    for (const addition of expansionCards.sstrAdditions)
      sstrCardIds.push(addition);
    return {
