...
export class WotrFrontUi {
  ...
  async useElvenRing(
    ...
    const toDieResult = await this.ui.askOption(
      'Choose a die to change to',
      toDieOptions,
    );
    const toDie: WotrActionDie =
      typeof fromDie === 'string'
        ? toDieResult
        : { type: fromDie.type, result: toDieResult };
