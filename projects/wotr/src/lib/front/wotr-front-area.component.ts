    ...
    <mat-tab-group>
      ...
      <mat-tab label="Reinforcements">
        <div class="reinforcements">
          @for (nation of nations (); track nation.id) { @for (i of (nation.reinforcements.regular | bgTransform:range);
          track i) {
          <img
          ...
          <img
            ...
            matTooltip="Nazgul" />
          } } @for (character of frontCharacters (); track character.id) { @if (character.status === "available") {
          <img
      ...
      <mat-tab label="Casualties">
        <div class="casualties">
          @for (nation of nations (); track nation.id) { @for (i of (nation.casualties.regular | bgTransform:range);
          track i) {
          <img
            [src]="nation.id | bgTransform: armyUnitImage:'regular'"
            [matTooltip]="nation.regularLabel" />
          ...
          <img
            [src]="nation.id | bgTransform: leaderImage"
            [matTooltip]="nation.leaderLabel" />
          } } @for (character of frontCharacters (); track character.id) { @if (character.status === "eliminated") {
          <img
            [src]="character.id | bgTransform: characterImage"
            [matTooltip]="character.name" />
          } }
...
export class WotrFrontAreaComponent {
  ...
  protected armyUnitImage: BgTransformFn<WotrNationId, string, WotrArmyUnitType> = (nationId, type) =>
