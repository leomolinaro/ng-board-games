...
export class WotrMinionStore {
  ...
  setEliminated (minionId: WotrMinionId) {
    this.updateMinion ("setEliminated", minionId, minion => ({
      ...minion,
      status: "eliminated"
    }));
  }

  setInPlay (minionId: WotrMinionId) {
    this.updateMinion ("setInPlay", minionId, minion => ({
      ...minion,
      status: "inPlay"
    }));
  }
