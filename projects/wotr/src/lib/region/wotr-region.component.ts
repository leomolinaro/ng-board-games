...
export class WotrRegionComponent {
  ...
  regionNode: Signal<WotrRegionNode> = computed (() => {
    return this.regionToNode (this.region (), null);
  });
  ...
  private equalsRegionUnits (a: WotrRegion, b: WotrRegion) {
    ...
    if (a.freeUnits !== b.freeUnits) { return false; }
  ...
  private regionToFreeGroups (region: WotrRegion): WotrFreeGroupNode[] {
    ...
    if (region.fellowship) {
      freeGroups.push ({ units: [{ unitType: "fellowship", image: this.assets.getFellowshipImage (false), svgX: 0, svgY: 0 }], nUnits: 1, svgX: 0, svgY: 0 });
    }
