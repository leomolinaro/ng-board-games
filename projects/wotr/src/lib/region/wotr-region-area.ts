...
interface WotrFreeGroupNode {
  units:
    | WotrFellowshipNode[]
    | WotrFreePeoplesFreeUnitNode[]
    | WotrShadowFreeUnitNode[];
  nUnits: number;
  svgX: number;
  svgY: number;
}
...
export class WotrRegionArea {
  ...
  private regionToArmyNode(region: WotrRegion): WotrArmyNode | null {
    ...
    const [armyUnits, nRegulars, nElites] = this.regionToArmyUnitNodes(
      region.army,
    );
    ...
    switch (armyFront) {
      case 'free-peoples':
        [leaderUnits, leadership] =
          this.regionToFreePeopleLeaderUnitNodes(region);
        break;
      case 'shadow':
  ...
  private regionToFreePeopleFreeUnitNodes(
    ...
    const unitNodes = fpFreeUnits
      .slice(0, 2)
      .map<WotrFreePeoplesFreeUnitNode>((freeUnit) => ({
        unitType: 'companion',
