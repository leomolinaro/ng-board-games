  ...
  return {
    ...
    map: {
      ...
      southrons: initialShadowNation(
        ...
        6,
        0,
        'Southrons & Esterlings regular',
        'Southrons & Esterlings elite',
      ),
    },
  };
}
...
export class WotrNationStore {
  ...
  freePeoplesNations = computed(() => {
    const s = this.state();
    return s.fpNationIds.map((id) => s.map[id]);
  });
  shadowNations = computed(() => {
    const s = this.state();
  ...
  private updateUnitCasualties(
    ...
    this.updateNation('updateUnitCasualties', nationId, (nation) => ({
      ...
      casualties: {
        ...nation.casualties,
        [unitType]: nation.casualties[unitType] + deltaQuantity,
      },
    }));
  }
  ...
  private getPreviousPoliticalStep(
    politicalStep: WotrPoliticalStep,
  ): WotrPoliticalStep {
    switch (politicalStep) {
      case 'atWar':
