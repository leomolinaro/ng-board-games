...
export class WotrNationModifiers {
  public readonly canAdvanceNationModifier =
    new WotrModifier<WotrCanAdvanceNationModifier>();
  public canAdvanceNation(
    nationId: WotrNationId,
    source: WotrNationAdvanceSource,
  ): boolean {
    return this.canAdvanceNationModifier
      .get()
      .every((modifier) => modifier(nationId, source));
  }

  public readonly afterNationAdvance =
    new WotrModifier<WotrAfterNationAdvance>();
  public onAfterNationAdvance(
    nationId: WotrNationId,
    source: WotrNationAdvanceSource,
  ): void {
    for (const handler of this.afterNationAdvance.get())
      handler(nationId, source);
  }

  public readonly canActivateNationModifier =
    new WotrModifier<WotrCanActivateNationModifier>();
  public canActivateNation(
    nationId: WotrNationId,
    source: WotrNationActivationSource,
  ): boolean {
    return this.canActivateNationModifier
      .get()
      .every((modifier) => modifier(nationId, source));
  }

  public readonly afterNationActivation =
    new WotrModifier<WotrAfterNationActivation>();
  public onAfterNationActivation(
    nationId: WotrNationId,
    source: WotrNationActivationSource,
  ): void {
    for (const handler of this.afterNationActivation.get())
      handler(nationId, source);
  }
