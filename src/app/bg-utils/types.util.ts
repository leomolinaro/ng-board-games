...
export function isEmptyKey (key: any) { return !(key || key === 0); }

export function isArray<T> (value: any): value is T[] { return Array.isArray (value); }
export function isString (value: any): value is string { return typeof value === "string"; }
export function isArrayOfStrings (value: any[]): value is string[] { return !value || !value.length || value.every (v => isString (v)); }

export type KeyOfType<T, TProp> = { [P in keyof T]: T[P] extends TProp ? P : never }[keyof T]; // funziona?
