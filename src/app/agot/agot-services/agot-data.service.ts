...
import { AgotCard, AgotFaction, AgotPack, AgotPackCode, AgotType, AgotTypeCode } from "../agot.models";
...
export class AgotDataService {
  ...
  private officialPackCodes: Record<AgotPackCode, boolean> = {
    "Core": true,
    "WotN": true, "LoCR": true, "WotW": true, "HoT": true, "SoD": true,  "KotI": true, "FotS": true, "DotE": true,
    "TtB": true, "TRtW": true, "TKP": true, "NMG": true, "CoW": true, "TS": true,
    "AtSK": true, "CtA": true, "FFH": true, "TIMC": true, "GoH": true, "TC": true,
    "AMAF": true, "GtR": true, "TFoA": true, "TRW": true, "OR": true, "TBWB": true,
    "TAK": true, "JtO": true, "Km": true,  "FotOG": true, "TFM": true, "SAT": true,
    "TSC": true, "TMoW": true, "SoKL": true, "MoD": true, "IDP": true, "DitD": true,
    "AtG": true, "CoS": true, "PoS": true, "BtRK": true, "TB": true, "LMHR": true
  }; // officialPackCodes
  ...
  getFactions$ () { return this.$factions.asObservable (); }
  getPacks$ (options?: { onlyOfficial: boolean }) { return this.$packs.asObservable ().pipe (map (p => this.filterPacks (p, options))); }
  getTypes$ () { return this.$types.asObservable (); }

  getCards (options?: { onlyOfficial: boolean }) { return this.filterCards (this.cards!, options); }
  getPacks (options?: { onlyOfficial: boolean }) { return this.filterPacks (this.$packs.getValue (), options); }
  getCard (cardCode: string) { return this.cardMap[cardCode]; }
  getFactions () { return this.$factions.getValue (); }
