...
import { BaronyBoardService } from "../barony-board/barony-board.service";
...
export class BaronyLogComponent implements OnChanges {
  ...
  ngOnChanges (changes: SimpleChanges<BaronyLogComponent>) {
    if (changes.log) {
      ...
      switch (l.type) {
        ...
        case "construction": this.fragments = [this.player (l.player), this.string (" constructs a "), this.pawn (l.construction.building), this.string (" in "), this.land (l.construction.land), this.string (".")]; break;
  ...
  private player (playerId: string): BaronyLogPlayerFragment {
    const player = this.service.getPlayer (playerId);
    return {
      type: "player",
      label: player.name,
      player: player
  ...
  private land (landId: BaronyLandCoordinates): BaronyLogLandFragment {
    const land = this.service.getLand (landId);
    let label: string;
    switch (land.type) {
